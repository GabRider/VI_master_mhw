      ...
      <tr>
        ...
        <td>
            <div v-if="piece.slots.length==0">
                <span class="badge badge-primary">jewels: </span> <img  src="../assets/empty.png"  width="24" height="24"/>
            </div>
  ...
  methods: {
    ...
    selectPiece() {
      ...
      if (!this.myCurrentArmorSet.some((a) => a.id === this.piece.id)) {
        let newArmorSet = this.myCurrentArmorSet.filter(
          (e) => e.type !== this.piece.type
        );
    ...
    unselectPiece() {
      ...
      if (this.myCurrentArmorSet.some((a) => a.id === this.piece.id)) {
        const newArmorSet = this.myCurrentArmorSet.filter(
          (a) => a.id !== this.piece.id
        );
        this.$emit("update:myCurrentArmorSet", newArmorSet);
      }
    },
